...
def find_vulnerability_in_func(node,func,tainted_vars,logs,filename): 
    ...
    if node.name in ('IfStatement','ForStatement','WhileStatement'):
        ...
        if len(taintedparams)>0 and calleename!="" and calleename!=None:
            func.children.append({'name':calleename,'taintedparam':taintedparams})
    ...
    if node.name=='CallExpression':
        for child in node.children:
            if child.body=='arguments':
                arguments=child
            elif child.body=='callee':
                callee=child
    if callee and callee.name=='MemberExpression':
        caller=None
        member=None
        for child in callee.children:
            if child.body=='object':
                caller=child
            elif child.body=='property':
                member=child
        
        if str(get_node_computed_value_e(caller))=='window' and str(get_node_computed_value_e(member))=='define':
        #     # if member.name=='CallExpression':
        #     print(member.name,get_node_computed_value_e(member),vars(member))
...
def analyze_a_wxapkg(wxid):
    ...
    walk_dir='/home/allen/Github/SP23/DoubleX/src/output/'+wxid+'/'
...
"setStorage",
"setData"]
    # walk_dir='/home/allen/Github/SP23/DoubleX/src/output/zzzVlb8U0sn26sxTtLPMVusanEpqBInZ'
    alljsfiles=[]
    subfolder=os.listdir(walk_dir)[0]
    
    
    functioncallmap={}
    fn=walk_dir+subfolder+"/app.js"
    print(fn)
    ...
    for filename in funcfilemap:
        ...
        for func in reversefuncmap:
            for child in reversefuncmap[func]['param_connected_by']:
                ...
                # if filename=="231":
                #     print("_____",filename,func in funcmap,"_____",func,'paramatically linked by',child,"||functions called by",child, "with extradata transfered: "+calledlist)
        # if filename=="231":
        #     print("!?!",funcmap['updatePValue'])
        for func in funcmap:
    ...
    # print((time.time()-t))
# print (nodes)
def analyze_from_wxapkg(wxid):
    # batch='22'
    # wxid='wxcd538bb3d50152f1'
    print("ANALYZING FROM BAIDUAPKG")
    analysis_res,exec_time=analyze_a_wxapkg(wxid)
    finalresult={'wxid':wxid,'res':analysis_res,'exec_time':exec_time}
    return finalresult
# print (analyze_from_wxapkg('0AK7M9YjlxChMgZdu81SG414t5RUnhpt'))
# print(time.time())
#l,e=analyze_a_wxapkg('zYMRIjN2b1KY0uG4610p5v20Tqsyvxzc')
#print(l)
# print(l)
# print(time.time())
